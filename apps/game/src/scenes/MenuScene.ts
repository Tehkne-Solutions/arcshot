...
export class MenuScene extends Phaser.Scene {
  ...
  private getFeaturedCharacters(catalog: AssetCatalog): CharacterDefinition[] {
    const characters = FEATURED_CHARACTERS
    ...
    for (const character of catalog.characters) {
      ...
      if (!characters.some((item) => item.id === character.id)) characters.push(character);
    }
  ...
  private drawBackdrop(palette: ArcshotUiPalette): void {
    ...
    ornament.lineStyle(2, palette.borderStrong, 0.18).strokeCircle(1160, 610, 78).strokeCircle(1160, 610, 50);
  ...
  private drawHeader(palette: ArcshotUiPalette, themeName: ArcshotTheme, totalStars: number): void {
    ...
    this.add.text(146, 50, "TEHKNÉ SOLUTIONS", {
      fontFamily: "Lexend", fontSize: "12px", fontStyle: "bold", color: palette.isLight ? "#2f6cff" : "#67dcff", letterSpacing: 2.6,
    });
  ...
  private drawCampaign(palette: ArcshotUiPalette, missionStars: number[], selectedId: string): void {
    ...
    MISSIONS.forEach((mission, index) => {
      ...
      this.add.circle(
        ...
        y + 32,
        19,
        completed ? palette.success : unlocked ? palette.primary : palette.disabledFill,
        unlocked ? 1 : 0.55,
      );
  ...
  private drawCombatants(
    ...
    featured.forEach((character, index) => {
      ...
      const card = this.add.rectangle(x, y, 124, 210, selected ? palette.panelStrong : palette.panelAlt, 1)
        .setOrigin(0)
      ...
      this.add.image(x + 62, y + 71, art.textureKey).setDisplaySize(art.premium ? 108 : 104, art.premium ? 108 : 104);
      ...
      this.add.text(x + 62, y + 191, selected ? "SELECIONADO" : "ESCOLHER", {
    ...
    this.panel(808, 594, 406, 62, palette, palette.panelAlt);
